...
Otra manera de visualizarlo, de manera más clara, en un sólo gráfico:




```{r}
# pivotear para poder poner dos variables en el eje Y, o sea los porcentajes.
areas_palma%>%
  pivot_longer(cols = c(porcentaje,porcentaje_ideal),names_to = "porcentaje",values_to = "porcentaje_comparado")%>%
  ggplot(aes(x=clase,y=porcentaje_comparado,fill=porcentaje))+
  geom_col(position="dodge",alpha=.6,color="black")+
  scale_fill_manual(values = c("porcentaje" = "#68228B", "porcentaje_ideal" = "orange3"),
                    labels=c("porcentaje" = "Observado", "porcentaje_ideal" = "Ideal"))+
  geom_text(aes(label = porcentaje_comparado), 
          position = position_dodge(width = 1), 
          vjust = -0.4, size = 3.5)+
  theme_linedraw()+
  labs(title = "Participación del ingreso total: Chile 1988 vs Caso teórico 'ideal'",
       x="Clase",
       y="Porcentaje",
       fill="Caso")
```
